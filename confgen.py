...
from logging import basicConfig, info, warning, error, INFO, StreamHandler
...
for corpCfgFilename in listdir(PATH_REGISTRY):
    if corpCfgFilename not in corporaCfg:
        warning(f'skipping {corpCfgFilename} because it is not defined in the corpora.yaml')
        continue
    ...
    corpCfgStr = [x for x in corpCfgStr if x.startswith('ATTRIBUTE') or x.startswith('STRUCTURE') or x.startswith('}')]
    ...
    state = 'none'
    bracketsCount = 0
    for i in finditer(r'}|@(STRUCTURE|ATTRIBUTE)\s+([-a-zA-Z0-9_]+)\s*({)?', corpCfgStr):
        if i.group(0) == '}':
            bracketsCount -= 1
        else:
            if i.group(3) == '{':
    ...
    for k, v in cfg['structureMapping'].items():
        if v not in structures:
            error(f'erorr in mapping {v} as {k} - {v} is not among defined structures ({", ".join(structures)})')
            valid = False
    for i in ['title', 'landingPageURI', 'utterance', 'paragraph', 'turn', 'text', 'session']:
        cfg.pop(i)
    # attributes
    cfg['posAttrs'] = []
    for x in attributes:
        if x in ATTR_TO_LAYER:
            ...
            warning(f'skipping attribute {x} as it lacks mapping to an FCS layer (text/lemma/pos/orth/norm/phonetic)')
