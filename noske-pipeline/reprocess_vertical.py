...
from create_verticals import create_config, create_mquery_sru_config, run_udp
    ...
    for row in input:
        ...
        if text == GLUE:
            ...
            if tag in tagsC:
                ...
                time["tokens"] += tokens
                print(
                    f"    nlp {round(t3 - t2, 2)}s total {round(t3 - t1, 2)}s tokens {tokens}"
                )
...
def main():
    ...
    with open(args.vertical) as infile:
        ...
        if not args.co:
            for i in range(10):
                with open(
                    join(cfg["outputDir"], basename(args.vertical)), "w"
                ) as outfile:
